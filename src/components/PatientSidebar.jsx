...
const PatientSidebar = () => {
    ...
    const navItems = [
        ...
        { id: DASHBOARD_TABS.ECG_UPLOAD, label: 'آپلود نوار قلب', icon: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        { id: DASHBOARD_TABS.HISTORY, label: 'تاریخچه آزمایش‌ها', icon: <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        { id: DASHBOARD_TABS.PROFILE, label: 'پروفایل', icon: < UserIcon/> }
    ]
                ...
                <nav>
                    <ul className="flex flex-col gap-[0.25rem] leading-[1.25rem] pt-[0.5rem] text-[0.875rem]">
                        {navItems.map((item) => (
                            <li 
                                key={ item.id }
                                className={`${ activeTab === item.id ? 'bg-primary/20 font-semibold rounded-[1.15rem] text-accent' : 'bg-white font-normal rounded-[1.15rem] text-text-muted-foreground' }`}
                                onClick={() => setActiveTab(item.id)}
                            >
                                <Link className={`flex gap-[0.75rem] items-center pr-[0.75rem] py-[0.625rem]`}>
                                    { item.icon }
